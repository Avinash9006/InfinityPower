...
const createChapter = async (req, res) => {
  try {
    ...
    const { title, description, videos, order, resources } = req.body;
    if (!title) {
      return res
        .status(400)
      ...
      await Promise.all(
        resources.map(async (resourceId) => {
          await Resource.findOneAndUpdate(
            { _id: resourceId, tenantId: req.user.tenantId },
            { chapterId: chapter._id }
          );
        })
      );
...
const getChapters = async (req, res) => {
  try {
    ...
    if (!subjectId) {
      return res
        .status(400)
        .json({ success: false, message: "Subject ID required" });
    }

    const chapters = await Chapter.find({ subjectId }).sort({
      order: 1,
      createdAt: 1,
    });
...
const getChapterById = async (req, res) => {
  try {
    ...
    if (!id) {
      return res
        .status(400)
        .json({ success: false, message: "Chapter ID required" });
    }
